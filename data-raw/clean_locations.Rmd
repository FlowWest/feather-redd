---
title: "feather river redd survey locations clean uo"
date: "2024-11-06"
output: 
  html_document:
    theme: flatly
...
fixed_zeros <- read.csv("data-raw/Faulty_lat_long_records_edits_Chris_110524.csv") |> # csv with Chris' notes
  select(-1) |> 
...
#TODO note that there is one latitude that still seems off - removing it for now
fixed_zeros |> 
  filter(latitude > 39.99) |> view()

fixed_zeros <- fixed_zeros |> 
  filter(!(date == "10/6/2014" & survey_wk == "5-1" & file_number == "77")) |> #removing that entry for now
  glimpse()
...
```
#TODO plot to check locations now

#TODO need the original raw data file to be able to use same criteria to filter out data entries
