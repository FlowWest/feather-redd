...
```{r}
problematic_records <- redd_observations_clean_2 |> 
...
```{r}
ggplot(redd_observations_wnames, aes(x = date, y = number_redds)) +
  geom_point()
```
...
```{r}
summary <- redd_observations_wnames |> 
  group_by(location, date) |> 
  summarize(number_redds) |> 
  glimpse()

redd_summary_by_date <- redd_observations_wnames |> 
  group_by(location, date) |> 
...
redd_observations_wnames <- redd_observations_wnames |> 
  mutate(year = year(date), week = week(date))

redd_summary_by_year_week <- redd_observations_wnames |> 
