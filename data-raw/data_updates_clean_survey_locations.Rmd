...
Listing redd location names that remain unmatched to any survey locations
...
Map with redd points located outside of survey locations (and naming does not match any survey location)
...
# Add circle markers with permanent labels
locations <- unique(names_for_check_2$location)
name_check_map_2 <- name_check_map_2 |> 
  addCircleMarkers(
    data = names_for_check_2, 
    ~longitude, ~latitude, 
    label = ~location,
    labelOptions = labelOptions(noHide = TRUE), # Permanent label display
    popup = ~paste("Location:", location, "<br>Date:", date, "<br>Latitude:", latitude, "<br>Longitude:",
                   longitude, "<br>Survey Week:", survey_wk),
    group = ~location,
    radius = 2,  
...
# Add layers control (unchanged)
name_check_map_2 <- name_check_map_2 |> 
  addLayersControl(
    overlayGroups = c(locations), # Add checkboxes for the location names 
    options = layersControlOptions(collapsed = FALSE)
  )
