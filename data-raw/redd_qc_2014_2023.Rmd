...
- 2014-10-20 has 6 records with high numer_salmon numbers (over 15, check if these are not errors)
...
sum(combined_14_22_utm$longitude > -100)
...
data_14_22 <- combined_14_22_utm |> 
  mutate(latitude = ifelse(latitude < 38, 0, latitude),
         longitude = ifelse(longitude > -100, 0, longitude))
...
```{r}
# ggplot(combined_redd, aes(date, location)) +
#   geom_point() +
#   theme_minimal() +
...
ggplot(combined_redd, aes(x = month(date), y = depth_m)) +
  geom_boxplot() +
...
#TODO
```{r}
#filtered out those that are NA for lat/long
redd_coords <- combined_redd |> 
   filter(!is.na(longitude_m_e) & !is.na(latitude_m_n)) |>  
  view()

#filter out those that seem to off UTM format
redd_coords_issue <- redd_coords |> 
  filter(longitude_m_e >= 0 | latitude_m_n >= 40) |>
  glimpse()

utm_crs <- 32610  # EPSG code for UTM zone 10N (WGS84)

# Create an sf object
sf_data <- st_as_sf(redd_coords_issue, coords = c("longitude_m_e", "latitude_m_n"), crs = utm_crs)
latlong_crs <- 4326  # EPSG code for WGS84
latlong_data <- st_transform(sf_data, crs = latlong_crs)

coords <- st_coordinates(latlong_data)

# Add the longitude and latitude as separate cols
latlong_data$longitude <- coords[, 1]  
latlong_data$latitude <- coords[, 2]   

latlong_data <- st_drop_geometry(latlong_data) |> 
  mutate(longitude = ifelse(longitude >= -120, NA, longitude),
         latitude = ifelse(latitude <= 39, NA, latitude)) |>
  glimpse()
# ---- second troubleshoot

# fix <- latlong_data |> 
#   filter(longitude >= -120 |
#          latitude <= 38) |> #filter out those that seem to off UTM format
#   glimpse()

#filtering data with UTM coordinates
redd_UTM <- combined_redd |> 
  filter(longitude_m_e <= 0 | latitude_m_n <= 40) |> 
  mutate(longitude = longitude_m_e,
         latitude = latitude_m_n) |> 
  select(-c(latitude_m_n, longitude_m_e)) |> 
  glimpse()

#binding data transformed to UTM and original UTM data
combined_redd_UTM <- bind_rows(latlong_data) |> 
  glimpse()

#todo add data that had lat long as NA

```

```{r}
#plot points 
palette <- viridis(n = nlevels(factor(combined_redd$location))) 

# leaflet(combined_redd_UTM) |> 
#   addTiles() |> 
#   addCircleMarkers(
#     ~longitude, ~latitude,
#     popup =  ~paste0("ID: ", location, "<br>Location: ", location),
#     radius = 2,
#     color = ~palette[as.numeric(factor(location))],
#     label = ~as.character(location),
#     labelOptions = labelOptions(noHide = TRUE, textOnly = TRUE)
#   )
leaflet(combined_redd) |> 
  addTiles() |> 
  addCircleMarkers(
    ~longitude, ~latitude,
    # popup = ~paste0("ID: ", location, "<br>Location: ", location),
...
filter(combined_redd, pot_depth_m > 10)
summary(combined_redd$velocity_m_s)
```

```{r, include = F}
# edits to data
combined_redd_final <- combined_redd |> 
...
write_csv(combined_redd_final, "data/all_redd_data.csv")
```
