...
write_csv(combined_redd_clean, "data/archive/all_redd_data.csv")

combined_redd_clean <- combined_redd_final |> #TODO confirm with Ashley that this is what she did manually
  select(-c(survey_wk, location, file_number)) 

# clean data to data/ with fields removed suggested by feather river team
write_csv(combined_redd_clean, "data/redd_observations.csv")
```

```{r, include=FALSE}
#Testing if we can pull polygons from Chris' map to categorize redd data

# gdb_path <- "data-raw/shapefiles/b105_redd_survey_locations_maps_2021.gdb"
# 
# st_layers(gdb_path)
# 
# layer_name <- "UpperHatchery"
...
# 
# # Remove the Z dimension (convert from 3D to 2D geometry)
...
#   addTiles() |> 
#   addPolygons(
#     fillColor = "lightblue",  # Customize the fill color
#     color = "darkblue",       # Border color of polygons
...
### Plotting survey locations from Chris' map

The following layers are included on map:
...
### Adding the points where redds are found to the survey locations
...
survey_locations_map_combined <- survey_locations_map |> 
  addCircleMarkers(
    ...
    color = "red",  # Change the color of the points
    stroke = FALSE,  # Remove border
    fillOpacity = 0.7  # Adjust fill opacity
  )

survey_locations_map_combined #TODO look into lower auditorium points. There are two that seem off locations 
```
