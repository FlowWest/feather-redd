...
#TODO
```{r}
combined_redd_filtered <- combined_redd |> 
  filter(longitude_m_e <= 0 | latitude_m_n <= 40) |> 
  glimpse()
  

redd_coords <- combined_redd |> 
   filter(!is.na(longitude_m_e) & !is.na(latitude_m_n)) |>  #filtered out those that are NA for lat/long
  view()

redd_coords_issue <- redd_coords |> 
  filter(longitude_m_e >= 0 | latitude_m_n >= 40) |> #filter out those that seem to off UTM format
  glimpse()
