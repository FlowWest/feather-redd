...
feather_redd_raw |> 
  ...
  theme_minimal() +
  theme(strip.background = element_rect(fill = "lightgrey", color = "grey50"))
```
...
The lack of precision and availability on latitude and longitude coordinates limits the accuracy of spatial analyses. We might be able to analyze data at a broader geographic scale, for example by area names, rather than specific locations. It might be challenging to precisely define spawning habitat criteria with the absence of substrate data completeness.

We might be able to use the data to calculate redd density as the redds per unit area
