...
feather_redd_raw |> 
  ...
  ggplot(aes(x = year, y = location, color = location)) +
...
Note that after 2015 that there isn't any point methods anymore
...
ggplot(feather_redd_raw, aes(x = date, y = salmon_count)) +
  geom_col() +
  labs(title = "Salmon Count by Date",
       x = "Date",
       y = "Salmon Count") +
  theme_minimal() 
...
The lack of precision and availability of latitude and longitude coordinates limits the accuracy of spatial analyses. Without precise coordinates, we might be able to analyze data only at a broader geographic scale, for example by area names, rather than specific location points. This limitation affects our ability to pinpoint exact spawning sites and accurately map spawning habitat. It might be challenging to precisely define spawning habitat criteria in the absence of complete substrate data, which is critical for understanding the suitability of spawning environments. Furthermore, we will have limited ability to directly correlate redd density with specific environmental factors such as substrate type, depth, and velocity due to the missing data on these parameters. This gap makes it difficult to fully understand the habitat of Chinook salmon.

However, despite these limitations, we might be able to use the available data to calculate redd density as the number of redds per unit area. By delineating surveyed areas based on available location names or river reaches, we can estimate redd density and assess patterns over time. Additionally, we can analyze trends in abundance (harvest plus escapement) over multiple years (> 3 years), comparing current data with historical baselines from periods and recent years. These analyses can provide insights into changes in population dynamics and the effectiveness of habitat enhancement projects, even though the precision of habitat characterization may be limited.