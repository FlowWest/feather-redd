...
### Summary of notes and observations of this markdown:
  ...
  - Survey location names and redd locations are inconsistent, this could be a spelling difference, or similar words. Question: is it worth spending time on unifying names across these two data sets?
  - Observations:
  ...
  - Most of the redd sites that are "off" a site location are off 
...
```
Note: Survey location names and redd locations are inconsistent, this could be a spelling difference, or similar words. Question: is it worth spending time on unifying names across these two data sets?
...
## Redd locations that are off stream, or that they do not fall within a locations site {.tabset} 
...
Filtering those redds that are off survey locations

- Observations:
  - There a lot more survey location names on the redd survey data, than the actual site names (some might be simply different spelling)
  - Most of the redd sites that are "off" a site location are off 
  
  - Locations that are off-channel and hard to relate to survey sites:
  ...
  - Locations that are very off distance from stream
  ...
  - All others are geographically closer to at least one of the sites
  - There are some redds conglomerated in the stream that do not fall within a survey site, however they all have "great western" as location (between Upper Robinson and Aleck). Question: are we missing that site from the survey's sites?
  - Similar that previous point for "lower vance east/lower vance"


Same map than above but adding sites
...
not_overlap_map <- not_overlap_map %>%
  addCircleMarkers(
    ...
    popup = ~paste0("Redds not in polygons<br>Number of Redds: ", number_redds, "<br>Location: ", location, "<br>Date: ", date, "<br>Survey Week: ", survey_wk),  # Popup info
    radius = 5,
    color = "red",
    stroke = FALSE,
    fillOpacity = 0.8,
